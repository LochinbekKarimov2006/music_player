...
function Playlist() {
  ...
  const malumod = JSON.parse(localStorage.getItem("malumod"));
  ...
  useEffect(() => {
    const getSpotifyPlaylists = async () => {
      try {
        const response = await axiosInstance.get(`playlists/${malumod.id}`);
        localStorage.setItem(
          "data",
          JSON.stringify(response.data.tracks.items)
        );
        setBarchaMalumodlar(response.data);
      } catch (error) {
  ...
  function keepTenWords(text) {
    const words = text.split(" ");
    let str = words.slice(0, 2).join(" ");
    ...
    <div className="">
      ...
      >
        ...
        <div className="flex pt-5 px-[21px] items-end gap-8">
          <div>
            <img
              className="max-w-[230px] max-h-[230px]"
              src={barchaMlaumodlar?.images[0]?.url}
              alt=""
            />
          </div>
          <div className="font-[500] text-[16px] text-[#fff]">
            <p>PUBLIC</p>
            <p>PLAYLIST</p>
            <h2 className="text-[80px] font-[800]">
              {keepTenWords(barchaMlaumodlar?.name)}
            </h2>
          </div>
        </div>
      </div>
      <div className="bg-[#2a2548] max-h-full pb-20">
        ...
        <div className="px-[25px]">
          <div className="overflow-x-auto">
            <table className="table">
              ...
              <tbody>
                {barchaMlaumodlar &&
                  barchaMlaumodlar?.tracks?.items.map((e, index) => (
                    <tr key={index} className="div-2 w-full">
                      <td className="flex items-center gap-3 ">
                        <p className="font-[600] text-[20px]">
                          {malumodSaqlash?.track?.id == e.track.id
                            ? "salom"
                            : index + 1}
                        </p>
                        <img
                      ...
                      <td></td>
                      <td className="text-[#fff] flex justify-end  font-[400] text-[18px] gap-0">
                        <button
